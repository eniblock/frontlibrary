...
import {SignedOperationGroup} from './TezosTypes';
 ...
 */
export class TezosMessageUtils {
    ...
    writeInt = (value: number): string => {
        ...
        // @ts-ignore
        return Buffer.from(Buffer.from(base128.encode(value), 'hex').map((v, i) =>
            i === 0 ? v : v ^ 0x80).reverse()).toString('hex');
    }
     ...
     * @param {number} value Number to be obfuscated.
     */
    writeSignedInt = (value: number): string => {
        ...
        return arr.map(elem => ('0' + elem.toString(16)).slice(-2)).join('');
    }
    ...
    readInt = (hex: string): number => {
        return base128.decode(
            // @ts-ignore
            Buffer.from(Buffer.from(hex, 'hex').reverse().map((v, i) => i === 0 ? v : v & 0x7f)).toString('hex')
        );
    }

    readSignedInt = (hex: string): number => {
    ...
    /**
     * Takes a hex string and reads a hex-encoded Zarith-formatted number starting at provided offset.
     * Returns the number itself and the number of characters that were used to decode it.
     * @param {string} hex Encoded message.
     * @param {number} offset Offset within the message to start decoding from.
     */
    findInt = (hex: string, offset: number, signed: boolean = false) => {
        let buffer = '';
        let i = 0;
        while (offset + i * 2 < hex.length) {
            const start = offset + i * 2;
            const end = start + 2;
            const part = hex.substring(start, end);
            buffer += part;
            i += 1;
        ...
        return signed ?
            { value: this.readSignedInt(buffer), length: i * 2 }
            : { value: this.readInt(buffer), length: i * 2 };
    }
    ...
    readAddress = (hex: string): string => {
        ...
        const implicitHint = hex.length === 44 ? hex.substring(0, 4) : '00' + hex.substring(0, 2);
        const implicitPrefixLength = hex.length === 44 ? 4 : 2;
    ...
    readAddressWithHint = (b: Buffer | Uint8Array, hint: string): string => {
        ...
        if (hint === 'tz1') {
            return this.readAddress(`0000${address.toString('hex')}`);
        } else if (hint === 'tz2') {
            return this.readAddress(`0001${address.toString('hex')}`);
        } else if (hint === 'tz3') {
            return this.readAddress(`0002${address.toString('hex')}`);
        } else if (hint === 'kt1') {
            return this.readAddress(`01${address.toString('hex')}00`);
        } else {
    ...
    readKeyWithHint = (b: Buffer | Uint8Array, hint: string): string => {
        ...
        if (hint === 'edsk') {
            ...
            return this.readPublicKey(`00${key.toString('hex')}`);
        } else {
     ...
     */
    computeOperationHash = (signedOpGroup: SignedOperationGroup): string => {
        const hash = this.cryptoUtils.simpleHash(signedOpGroup.bytes, 32);
        return this.readBufferWithHint(hash, 'op');
    }
     ...
     */
    computeKeyHash = (key: Buffer, prefix: string = 'tz1'): string => {
        const hash = this.cryptoUtils.simpleHash(key, 20);
        return this.readAddressWithHint(hash, prefix);
    }
}
